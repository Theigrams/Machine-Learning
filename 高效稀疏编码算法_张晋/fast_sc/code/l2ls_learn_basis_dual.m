...
if exist('Binit', 'var')
    ...
    dual_lambda = 10*abs(rand(M,1)); % any arbitrary initialization should be ok.  任意初始化都OK \(^o^)/~
end
...
%options = optimset('GradObj','on', 'Hessian','on');    此处由于版本不同无法运行，进行了修改如下：
...
if nargout > 1   % fun called with two output arguments  %当fmincon函数需要时，计算梯度
    ...
    if nargout > 2   %当fmincon函数需要时，计算Hessian阵
