...
if exist('batch_size', 'var') && ~isempty(batch_size)   %如果没定义batch_size的话，就设置为总数据量/10
...
% optimization loop    开始循环
while t < pars.num_trials
    t=t+1;
    start_time= cputime;    %记录开始时间
    
    stat.fobj_total=0;      %记录目标函数值
    stat.fresidue_total=0;      %记录重构误差
    stat.fsparsity_total=0;     %记录惩罚项值
    stat.var_tot=0;
    stat.svar_tot=0;
    ...
    for batch=1:(size(X,2)/pars.batch_size),
        ...
        if t==1 || ~pars.reuse_coeff
            if strcmp(pars.sparsity_func, 'L1') || strcmp(pars.sparsity_func, 'LARS') || strcmp(pars.sparsity_func, 'FS')
                %如果是选择了特征搜索法或LARS法
                S= l1ls_featuresign(B, Xb, pars.beta/pars.sigma*pars.noise_var);    %调用特征搜索法
            else
        ...
        % update basis   用拉格朗日对偶进行基B的学习
        B = l2ls_learn_basis_dual(Xb, S, pars.VAR_basis);
    end
    
    % get statistics
    stat.fobj_avg(t)      = stat.fobj_total / pars.num_patches;         %   计算平均目标函数值
    stat.fresidue_avg(t)  = stat.fresidue_total / pars.num_patches;  %   计算平均重构误差
    stat.fsparsity_avg(t) = stat.fsparsity_total / pars.num_patches;   %  计算平均惩罚项值
